...
public class KafkaJsonListenerService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(KafkaJsonListenerService.class);

    @KafkaListener(topics = Topics.SCORE_AFTER_ASSAY_TOPIC, containerFactory = "kafkaListenerJsonContainerFactory")
    public void receiveScoreAfterAssay(ConsumerRecord<String, Score> record, @Payload Score payload) {
        LOGGER.info("receive [{}]", record.toString());
    }
}
